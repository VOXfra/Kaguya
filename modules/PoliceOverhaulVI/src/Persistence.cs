    ...
    internal static class Persistence
    {
            ...
            try
            {
                ...
                foreach (XmlNode node in nodes)
                {
                    ...
                    var c = new CaseMemory
                    {
                        ...
                        FaceKnown = ReadBool(node, "FaceKnown", false),
                        OutfitKnown = ReadBool(node, "OutfitKnown", false),
                    ...
                    if (vehicle != null) c.Vehicle = new VehicleSignature { ModelHash = ReadInt(vehicle, "ModelHash", 0), Plate = ReadText(vehicle, "Plate", string.Empty), PlateKnown = ReadBool(vehicle, "PlateKnown", false), PrimaryColor = ReadInt(vehicle, "PrimaryColor", -1), SecondaryColor = ReadInt(vehicle, "SecondaryColor", -1), TrackerPresent = ReadBool(vehicle, "TrackerPresent", false), TrackerKnownByPolice = ReadBool(vehicle, "TrackerKnownByPolice", false) };
                    repository.Put(c);
                }
                repository.ClearExpired(); if (log != null) log("Persistent police cases loaded.");
            }
        ...
        public static void SaveCases(string path, CaseRepository repository, Action<string> log)
        {
            ...
            try
            {
                ...
                {
                    w.WriteStartDocument(); w.WriteStartElement("PoliceOverhaulVI"); w.WriteAttributeString("version", "0.1.0"); w.WriteStartElement("Cases");
                    foreach (CaseMemory c in repository.Cases)
                    {
                        if (c == null || (!c.Active && !c.WarrantActive && c.UnpaidFines <= 0)) continue;
                        w.WriteStartElement("Case");
                        Write(w,"SuspectModelHash",c.SuspectModelHash); Write(w,"Active",c.Active); Write(w,"FaceKnown",c.FaceKnown); Write(w,"OutfitKnown",c.OutfitKnown); Write(w,"ThreatLevel",c.ThreatLevel); Write(w,"HeatPoints",c.HeatPoints); Write(w,"ExpiresUtcTicks",c.ExpiresUtcTicks); Write(w,"WarrantActive",c.WarrantActive); Write(w,"WarrantExpiresUtcTicks",c.WarrantExpiresUtcTicks); Write(w,"LastKnownX",c.LastKnownX); Write(w,"LastKnownY",c.LastKnownY); Write(w,"LastKnownZ",c.LastKnownZ); Write(w,"LastSource",(int)c.LastSource); Write(w,"WeaponKnown",c.WeaponKnown); Write(w,"WeaponHash",c.WeaponHash); Write(w,"SuspectCountKnown",c.SuspectCountKnown); Write(w,"SuspectCount",c.SuspectCount); Write(w,"UnpaidFines",c.UnpaidFines);
