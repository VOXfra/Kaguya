...
namespace VOX.PoliceOverhaulVI
{
    public sealed class PoliceOverhaulVIScript : Script
    {
        ...
        private int _freeRoamOwnershipUntil;
        ...
        private void OnTick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                RefreshFreeRoamOwnership(nativeWanted);
                if (_cfg.MissionSafeMode && ShouldYieldToRockstar(nativeWanted))
                {
                ...
                if (_pending)
                {
                    if (nativeWanted > 0)
                    {
                        _pendingWanted = Math.Max(_pendingWanted, nativeWanted);
                        SetWantedLevel(0);
                        nativeWanted = 0;
                    }
                ...
                RefreshFreeRoamOwnership(_lastWanted);
            }
            catch (Exception ex)
            {
                Log("Tick error: " + ex);
        ...
        private void RefreshFreeRoamOwnership(int nativeWanted)
        {
            int now = Game.GameTime;
            bool active = _pending || nativeWanted > 0 || _lastWanted > 0 || _internalWanted > 0;
            if (!active && _case != null && _case.LastWantedEndedAt > 0)
                active = now - _case.LastWantedEndedAt < 5000;
            if (active) _freeRoamOwnershipUntil = Math.Max(_freeRoamOwnershipUntil, now + 15000);
        }
        ...
        private void BeginPendingIncident(int wanted)
        {
            ...
            ResetPendingSource();
            _freeRoamOwnershipUntil = Math.Max(_freeRoamOwnershipUntil, Game.GameTime + 15000);
        ...
        private void ProcessPendingIncident(Ped player)
        {
            ...
            if (_pendingSource != ObservationSource.None)
            {
                if ((_pendingSource == ObservationSource.Civilian || _pendingSource == ObservationSource.Police) &&
                    _pendingWitness != null && _pendingWitness.Exists() && _pendingWitness.IsDead)
                    ResetPendingSource();

                if (_pendingSource != ObservationSource.None && now - _pendingSourceSeenAt >= RequiredReportDelay(_pendingSource))
                {
        ...
        private void ResetPendingSource()
        {
            _pendingSourceSeenAt = 0;
            _pendingSource = ObservationSource.None;
            _pendingWitness = null;
            _pendingSourceDistance = float.MaxValue;
            _pendingCameraHandle = 0;
        }
        ...
        private void ObserveActiveWanted(Ped player, int wanted)
        {
            ...
            if (police != null)
            {
                CaptureEvidence(player, ObservationSource.Police, police.Distance, wanted);
                observed = true;
            }
            ...
            if (!observed && _cfg.CctvEnabled && now - _lastCameraScan >= 700)
            {
                ...
                if (camera != null)
                {
                    CaptureEvidence(player, ObservationSource.CCTV, camera.Distance, wanted);
                    observed = true;
                }
            }
            ...
            if (!observed && TrackerSystem.HasPoliceUsableTracker(_case, player, _cfg) && now - _lastTrackerPing >= _cfg.TrackerPingIntervalMs)
            {
                ...
                _case.Touch(_cfg);
                observed = true;
                Log("Tracked vehicle transmitted a location ping; vehicle location does not confirm driver identity.");
            }
        ...
        private void CaptureEvidence(Ped player, ObservationSource source, float distance, int wanted)
        {
            ...
            if (camera || distance <= _cfg.WeaponRecognitionDistance)
            {
                ...
                if (armed)
                {
                    _case.WeaponKnown = true;
                    _case.WeaponHash = SuspectSnapshot.CurrentWeaponHash(player);
                }
            }

            if (camera || distance <= _cfg.SuspectCountRecognitionDistance)
            {
                _case.SuspectCountKnown = true;
                _case.SuspectCount = SuspectSnapshot.CountVisibleSuspects(player);
            }
            IssueWarrantIfEligible();
        }
        ...
        private void IssueWarrantIfEligible()
        {
            ...
            if (_case.IdentityConfirmed && _case.ThreatLevel >= Math.Max(1, _cfg.WarrantMinimumThreat))
            {
                ...
                Log("Arrest warrant issued after identity confirmation (confidence=" + (int)_case.IdentityConfidence + "%).");
            }
        }
        ...
        private void FinishWantedPhase()
        {
            if (_case != null && _case.Active)
            {
                ...
                _case.Touch(_cfg);
                IssueWarrantIfEligible();
                if (_case.ThreatLevel >= 4)
                    IdentificationSystem.AddNotoriety(_case, _case.ThreatLevel >= 5 ? 4f : 2f, _cfg);
        ...
        private void TryReacquire(Ped player)
        {
            ...
            if (_case.IsWarrantExpiredUtc())
            {
                _case.WarrantActive = false;
                _case.WarrantExpiresUtcTicks = 0;
            }
            ...
            if (_case.WarrantActive && _case.IdentityConfirmed && _warrants.Update(player, _case, _cfg, Log))
            {
                RestoreWantedFromCase(player, ObservationSource.HomeSurveillance);
                return;
            }
        ...
        {
            int now = Game.GameTime;
            if (_reacquireCandidateKey != key)
            {
                _reacquireCandidateKey = key;
                _reacquireCandidateSince = now;
                _reacquireCandidateConfidence = confidence;
                return false;
            }
            if (confidence + 10f < _reacquireCandidateConfidence)
            {
                _reacquireCandidateSince = now;
                _reacquireCandidateConfidence = confidence;
                return false;
        ...
        {
            _reacquireCandidateKey = 0;
            _reacquireCandidateSince = 0;
            _reacquireCandidateConfidence = 0f;
        }

        private void MaintainHudPolicy()
        {
            if (_cfg.HidePoliceBlips && Game.GameTime - _lastHudRefresh >= 250)
            {
        ...
        private void SetWantedCentre(GTA.Math.Vector3 p)
        {
            try { Function.Call(Hash.SET_PLAYER_WANTED_CENTRE_POSITION, Game.Player.Handle, p.X, p.Y, p.Z); }
            catch { }
        }
        ...
        private void ExpireCurrentCaseIfNeeded()
        {
            ...
            {
                float notoriety = _case.Notoriety;
                int heists = _case.MajorHeistsKnown;
                bool most = _case.MostWanted;
                _case.ClearAll();
                _case.Notoriety = notoriety;
                _case.MajorHeistsKnown = heists;
                _case.MostWanted = most;
                _searchHud.Cleanup();
                Log("Police case expired; historical notoriety retained.");
            }
        }
