...
namespace VOX.PoliceOverhaulVI
{
    public sealed class PoliceOverhaulVIFieldSurrenderScript : Script
    {
        ...
        private int _releaseIgnoreAt;
        private bool _ignoreApplied;

        public PoliceOverhaulVIFieldSurrenderScript()
        {
            Interval = 40;
            Tick += OnTick;
            Aborted += OnAborted;
            Log("Field surrender 0.1.0 loaded: contextual surrender to visible nearby police.");
        }
        ...
        private void OnTick(object sender, EventArgs e)
        {
            try
            {
                if (_releaseIgnoreAt > 0 && Game.GameTime >= _releaseIgnoreAt)
                {
                    ReleaseIgnore();
                    _releaseIgnoreAt = 0;
                }
                ...
                if (wanted <= 0 || player.IsInVehicle()) { ResetHold(); return; }

                bool armed = false, shooting = false, aiming = false;
                float speed = 0f;
                try
                {
                    armed = Function.Call<bool>(Hash.IS_PED_ARMED, player.Handle, 7);
                    shooting = Function.Call<bool>(Hash.IS_PED_SHOOTING, player.Handle);
                    aiming = Function.Call<bool>(Hash.IS_PLAYER_FREE_AIMING, Game.Player.Handle);
                    speed = Function.Call<float>(Hash.GET_ENTITY_SPEED, player.Handle);
                }
                catch { }

                Ped officer = FindVisibleOfficer(player, 16f);
                if (officer == null || !officer.Exists() || shooting || aiming || speed > 2.3f)
                {
                    ResetHold();
                    return;
                }
                ...
                if (armed)
                {
                    ShowHelp("Rangez votre arme pour vous rendre.");
                    ResetHold();
                    return;
                }

                ShowHelp("Maintenez ~INPUT_CONTEXT~ pour vous rendre.");
                bool down = Pressed(Context);
                if (!down) { ResetHold(); return; }

                if (_holdStarted == 0 || _officerHandle != officer.Handle)
                {
                    _holdStarted = Game.GameTime;
                ...
                if (elapsed >= 350 && !_ignoreApplied)
                {
                    try
                    {
                        Function.Call(Hash.TASK_HANDS_UP, player.Handle, 3000, officer.Handle, -1, false);
                ...
                _releaseIgnoreAt = Game.GameTime + 2200;
                _holdStarted = 0;
                _officerHandle = 0;
                Notify("Vous vous rendez aux forces de l'ordre.");
                Log("Field surrender accepted by visible officer; active pursuit cleared while persistent evidence remains.");
            }
            catch (Exception ex)
            {
                Log("Field surrender tick error: " + ex.Message);
                ResetHold();
            }
        }
        ...
        {
            Ped[] peds;
            try { peds = World.GetNearbyPeds(player, radius); } catch { return null; }
            Ped best = null;
            float bestDistance = radius;
            foreach (Ped p in peds)
            {
                if (p == null || !p.Exists() || p.IsDead || !Perception.IsLawPed(p)) continue;
                float d = Perception.Distance(player.Position, p.Position);
                if (d >= bestDistance) continue;
                bool los = false;
                try { los = Function.Call<bool>(Hash.HAS_ENTITY_CLEAR_LOS_TO_ENTITY, p.Handle, player.Handle, 17); } catch { }
                if (!los) continue;
                best = p;
                bestDistance = d;
            }
        ...
        private void ResetHold()
        {
            _holdStarted = 0;
            _officerHandle = 0;
            if (_releaseIgnoreAt == 0) ReleaseIgnore();
        }

        private void ReleaseIgnore()
        {
            if (!_ignoreApplied) return;
            try { Function.Call(Hash.SET_POLICE_IGNORE_PLAYER, Game.Player.Handle, false); } catch { }
            _ignoreApplied = false;
        }
        ...
        private static void Log(string text) { try { Directory.CreateDirectory("scripts\\PoliceOverhaulVI"); File.AppendAllText(LogPath, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + " | " + text + Environment.NewLine); } catch { } }
    }
}
