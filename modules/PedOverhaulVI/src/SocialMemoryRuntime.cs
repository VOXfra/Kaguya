    ...
    internal static class SocialMemoryRuntime
    {
        public static void Update(Ped ped,PedState state,ScenePerception scene,IList<Ped> nearby,IDictionary<int,PedState> states,Config cfg)
        {
            ...
            if(scene.Kind!=SceneThreatKind.SocialWarning&&scene.Kind!=SceneThreatKind.CrowdFlight)
            {
                ...
                Remember(state,scene.Kind,scene.SourceHandle,confidence,direct,0,now);
                return;
            }
            ...
            if(scene.SourceHandle>0&&states!=null&&states.TryGetValue(scene.SourceHandle,out sourceState)&&sourceState.KnownThreatKind!=SceneThreatKind.None)
            {
                ...
                if(transferred>=14f)
                    Remember(state,sourceState.KnownThreatKind,sourceState.KnownThreatSourceHandle,transferred,false,Math.Min(8,sourceState.KnowledgeHops+1),now);
            }
            else
            {
                // The observer only knows that other people are reacting. Do not
                // invent a culprit from a crowd movement.
        ...
        private static void Remember(PedState s,SceneThreatKind kind,int source,float confidence,bool direct,int hops,int now)
        {
            ...
            s.KnownThreatKind=kind;s.KnownThreatSourceHandle=source;s.KnownThreatConfidence=Math.Max(0f,Math.Min(100f,confidence));s.KnowledgeWasDirect=direct;s.KnowledgeHops=hops;s.LastKnowledgeAt=now;
        }
        ...
        private static void UpdateCompanion(Ped ped,PedState s,IList<Ped> nearby,IDictionary<int,PedState> states,int now)
        {
            ...
            if(s.TrustedCompanionHandle>0)
            {
                ...
                if(existing!=null&&existing.Exists()&&Distance(ped,existing)<=7f)return;
                s.TrustedCompanionHandle=0;
            }

            Ped best=null;float bestD=3.4f;
            foreach(Ped other in nearby)
            {
                if(other==null||!other.Exists()||other.IsDead||other.Handle==ped.Handle)continue;
                PedState os;if(!states.TryGetValue(other.Handle,out os))continue;
                float d=Distance(ped,other);if(d>bestD)continue;
        ...
        {
            try{return Function.Call<bool>(Hash.IS_PED_FACING_PED,a.Handle,b.Handle,75f)&&Function.Call<bool>(Hash.IS_PED_FACING_PED,b.Handle,a.Handle,75f);}catch{return false;}
        }
        private static Ped FindPed(IList<Ped> peds,int handle){foreach(Ped p in peds)if(p!=null&&p.Exists()&&p.Handle==handle)return p;return null;}
        private static float Distance(Ped a,Ped b){try{return SituationModel.Distance(a.Position,b.Position);}catch{return 999f;}}
    }
}
