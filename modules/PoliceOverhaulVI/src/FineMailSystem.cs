...
namespace VOX.PoliceOverhaulVI
{
    internal sealed class FineMailSystem
    {
        ...
        public void Deliver(CitationRecord citation, int paid, int unpaid, Config cfg, Action<string> log)
        {
            ...
            if (cfg.FineMailEnabled)
            {
                try
                {
                    ...
                    Function.Call<int>(Hash.END_TEXT_COMMAND_THEFEED_POST_MESSAGETEXT,"CHAR_CALL911", "CHAR_CALL911", true, 4,cfg.FineMailSender, subject);
                    if (cfg.FineMailSound) Function.Call(Hash.PLAY_SOUND_FRONTEND, -1, "Text_Arrive_Tone", "Phone_SoundSet_Default", true);
                    if (log != null) log("Traffic fine mail delivered: " + subject + ".");
                }
                catch (Exception ex) { if (log != null) log("Traffic fine mail notification failed: " + ex.Message); }
            }
        }

        private static string BuildSubject(CitationRecord c) { return "Traffic citation - $" + Math.Max(0, c.Amount); }
        private static string BuildBody(CitationRecord c, int paid, int unpaid)
        {
            ...
            string payment = unpaid <= 0 ? "Fine paid automatically: $" + Math.Max(0, paid) + "." : "Paid: $" + Math.Max(0, paid) + ". Outstanding: $" + Math.Max(0, unpaid) + ".";
        ...
        private static void Archive(CitationRecord citation, string subject, string body)
        {
            try
            {
                ...
                string line = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " | " + subject + " | source=" + citation.Source + " | camera="+(citation.CameraId??string.Empty)+" | " + body + Environment.NewLine;
