...
namespace VOX.PoliceOverhaulVI
{
    public sealed class PoliceOverhaulVIRuntimeRepairScript : Script
    {
        ...
        public PoliceOverhaulVIRuntimeRepairScript()
        {
            ...
            Log("Police runtime repair 0.8.0 loaded: death reset and bounded search lifetimes.");
        }
        ...
        private void OnTick(object sender,EventArgs e)
        {
            try
            {
                ...
                {
                    try{if(Function.Call<int>(Hash.GET_PLAYER_WANTED_LEVEL,Game.Player.Handle)<=0)Function.Call(Hash.SET_POLICE_IGNORE_PLAYER,Game.Player.Handle,false);}catch{}
                }
            }
        ...
        private static void ResetActivePursuitOnDeath()
        {
            ...
            Log("Player death: active wanted/search state cleared. Historical case evidence remains persistent.");
        }

        private void OnAborted(object sender,EventArgs e){try{Function.Call(Hash.SET_POLICE_IGNORE_PLAYER,Game.Player.Handle,false);}catch{}}
