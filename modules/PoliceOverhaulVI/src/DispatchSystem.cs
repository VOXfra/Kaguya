...
namespace VOX.PoliceOverhaulVI
{
    internal sealed class DispatchSystem
    {
        ...
        private static DispatchSystem _active;
        ...
        public DispatchSystem(){_active=this;}

        // Death and field-surrender scripts are separate SHVDN Script instances.
        // This bridge lets them immediately delete VOX-owned military entities too,
        // rather than only clearing the vanilla wanted integer.
        public static void EmergencyCleanup()
        {
            DispatchSystem d=_active;
            if(d!=null)d.CleanupAll();
        }
        ...
        public void UpdateResponse(Ped player,int level,Config cfg,Action<string> log)
        {
            ...
            // 1..5: Rockstar owns every ground officer, vehicle, combat task and
            // pursuit task. RC3's extra retasking was the source of the frozen cops.
            if(level<6){if(_units.Count>0)CleanupMilitary();return;}

            // 6-star search: direct TASK_COMBAT/TASK_HELI_CHASE entities are removed
            // as soon as visual contact is genuinely lost. They may redeploy only
            // after PoliceSearchOwnership confirms a reacquisition.
            if(PoliceSearchRuntimeState.SearchActive)
            {
                if(_units.Count>0){CleanupMilitary();if(log!=null)log("Sixth-star military withdrawn for LOS search; direct combat tasks cleared.");}
                return;
            }

            if(!cfg.CustomDispatchEnabled||player==null||!player.Exists())return;
            int maxUnits=Math.Max(2,cfg.MaxCustomUnits),now=Game.GameTime;
            if(now-_lastHeavySpawn>=Math.Max(7000,cfg.SixStarHeavyIntervalMs)&&_units.Count<maxUnits)
            {
                ...
                if(cfg.SixStarAttackHelicopter&&_units.Count<maxUnits)spawned|=SpawnHelicopter(player,explosiveAllowed?"savage":"polmav",explosiveAllowed,log);
        ...
        private bool SpawnMilitaryGround(Ped player,bool explosivesAllowed,Action<string> log)
        {
            ...
            string vehicleName=tank?"rhino":"crusader";int vm=SafeHash(vehicleName),pm=SafeHash("s_m_y_marine_01");
            if(!EnsureModel(vm)||!EnsureModel(pm))return false;
            Vector3 pos=FindGroundSpawn(player,260f+_units.Count*22f);float heading=HeadingTo(pos,player.Position);var u=new CustomUnit{CreatedAt=Game.GameTime};
            try
            {
                int v=Function.Call<int>(Hash.CREATE_VEHICLE,vm,pos.X,pos.Y,pos.Z,heading,false,false);if(v==0)return false;u.VehicleHandle=v;
                ...
                int driver=Function.Call<int>(Hash.CREATE_PED_INSIDE_VEHICLE,v,6,pm,-1,false,false);
                if(driver!=0){u.PedHandles.Add(driver);SetupMilitaryPed(driver);Function.Call(Hash.SET_DRIVER_ABILITY,driver,1f);Function.Call(Hash.SET_DRIVER_AGGRESSIVENESS,driver,0.84f);if(tank)Function.Call(Hash.TASK_COMBAT_PED,driver,player.Handle,0,16);else Function.Call(Hash.TASK_VEHICLE_CHASE,driver,player.Handle);}
                if(!tank)for(int seat=0;seat<2;seat++){int p=Function.Call<int>(Hash.CREATE_PED_INSIDE_VEHICLE,v,6,pm,seat,false,false);if(p==0)continue;u.PedHandles.Add(p);SetupMilitaryPed(p);Function.Call(Hash.TASK_COMBAT_PED,p,player.Handle,0,16);}
                _units.Add(u);if(log!=null)log("Sixth-star military ground unit deployed: "+vehicleName+".");return true;
            }
            catch(Exception ex){if(log!=null)log("Sixth-star ground spawn failed: "+ex.Message);CleanupUnit(u);return false;}
        ...
        private bool SpawnHelicopter(Ped player,string modelName,bool armed,Action<string> log)
        {
            ...
            Vector3 p=player.Position;double a=(Game.GameTime%6283)/1000.0;Vector3 spawn=new Vector3(p.X+(float)Math.Cos(a)*430f,p.Y+(float)Math.Sin(a)*430f,p.Z+150f);var u=new CustomUnit{CreatedAt=Game.GameTime};
            try
            {
                int heli=Function.Call<int>(Hash.CREATE_VEHICLE,vm,spawn.X,spawn.Y,spawn.Z,HeadingTo(spawn,p),false,false);if(heli==0)return false;u.VehicleHandle=heli;
                ...
                int pilot=Function.Call<int>(Hash.CREATE_PED_INSIDE_VEHICLE,heli,6,pm,-1,false,false);if(pilot!=0){u.PedHandles.Add(pilot);SetupMilitaryPed(pilot);Function.Call(Hash.TASK_HELI_CHASE,pilot,player.Handle,0f,0f,0f);}
                if(armed)for(int seat=0;seat<=1;seat++){int gunner=Function.Call<int>(Hash.CREATE_PED_INSIDE_VEHICLE,heli,6,pm,seat,false,false);if(gunner==0)continue;u.PedHandles.Add(gunner);SetupMilitaryPed(gunner);Function.Call(Hash.TASK_COMBAT_PED,gunner,player.Handle,0,16);}
                _units.Add(u);if(log!=null)log(armed?"Sixth-star attack helicopter deployed.":"Sixth-star overwatch helicopter deployed.");return true;
            }
            catch(Exception ex){if(log!=null)log("Helicopter spawn failed: "+ex.Message);CleanupUnit(u);return false;}
            finally{ReleaseModel(vm);ReleaseModel(pm);}
        }

        private bool SpawnJet(Ped player,Action<string> log)
        {
            int vm=SafeHash("lazer"),pm=SafeHash("s_m_y_marine_01");if(!EnsureModel(vm)||!EnsureModel(pm))return false;Vector3 p=player.Position;Vector3 spawn=new Vector3(p.X-800f,p.Y-500f,p.Z+320f);var u=new CustomUnit{CreatedAt=Game.GameTime};
            try
            {
                int jet=Function.Call<int>(Hash.CREATE_VEHICLE,vm,spawn.X,spawn.Y,spawn.Z,HeadingTo(spawn,p),false,false);if(jet==0)return false;u.VehicleHandle=jet;
                ...
                int pilot=Function.Call<int>(Hash.CREATE_PED_INSIDE_VEHICLE,jet,6,pm,-1,false,false);if(pilot!=0){u.PedHandles.Add(pilot);SetupMilitaryPed(pilot);Function.Call(Hash.TASK_COMBAT_PED,pilot,player.Handle,0,16);}
                _units.Add(u);if(log!=null)log("Sixth-star jet deployed.");return true;
            }
            catch(Exception ex){if(log!=null)log("Jet spawn failed: "+ex.Message);CleanupUnit(u);return false;}
            finally{ReleaseModel(vm);ReleaseModel(pm);}
        }
        ...
        {
            for(int i=_units.Count-1;i>=0;i--){CustomUnit u=_units[i];bool remove=level<6||!EntityExists(u.VehicleHandle)||Game.GameTime-u.CreatedAt>360000;if(!remove&&player!=null&&player.Exists()){Vector3 p=GetPosition(u.VehicleHandle);if(p!=Vector3.Zero&&Perception.Distance(p,player.Position)>2300f)remove=true;}if(remove){CleanupUnit(u);_units.RemoveAt(i);}}
        }
