...
namespace VOX.PoliceOverhaulVI
{
    internal sealed class WarrantSystem
    {
        ...
        public bool Update(Ped player, CaseMemory memory, Config cfg, Action<string> log)
        {
            ...
            try
            {
                Function.Call(Hash.SET_VEHICLE_SIREN, _vehicleHandle, true);
                if (EntityExists(_driverHandle)) Function.Call(Hash.TASK_VEHICLE_CHASE, _driverHandle, player.Handle);
                if (EntityExists(_passengerHandle)) Function.Call(Hash.TASK_AIM_GUN_AT_ENTITY, _passengerHandle, player.Handle, 4000, false);
            }
            catch { }
            memory.LastKnownPosition = player.Position;
            memory.LastSource = ObservationSource.HomeSurveillance;
            memory.LastObservedGameTime = Game.GameTime;
            if (log != null) log("Home-surveillance unit identified wanted suspect.");
            return true;
        }
        ...
        private void TrySpawnStakeout(HomeProfile home, Action<string> log)
        {
            ...
            try
            {
                ...
                Function.Call(Hash.SET_ENTITY_AS_MISSION_ENTITY, _vehicleHandle, true, true);
                Function.Call(Hash.SET_VEHICLE_ON_GROUND_PROPERLY, _vehicleHandle);
                Function.Call(Hash.SET_VEHICLE_ENGINE_ON, _vehicleHandle, false, true, true);
                Function.Call(Hash.SET_VEHICLE_SIREN, _vehicleHandle, false);
                Function.Call(Hash.SET_VEHICLE_HANDBRAKE, _vehicleHandle, true);
        ...
        private static void SetupCop(int h)
        {
            ...
            try
            {
                ...
                Function.Call(Hash.GIVE_WEAPON_TO_PED, h, stun, 20, false, true);
